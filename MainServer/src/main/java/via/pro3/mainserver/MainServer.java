...
public class MainServer {
    public static void main(String[] args) throws Exception
    {
      Model model = new ModelManager();
        ...
        Server server =
                ServerBuilder.forPort(9090).addService(new ServerImpl()).addService(new PatientImpl(model)).addService(new RegisterPatientImpl(model)).addService(new LoginImpl(model)).build().start();
