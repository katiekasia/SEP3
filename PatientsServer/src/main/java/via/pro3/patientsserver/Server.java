...
public class Server
{
  ...
  public ResponseDto cancelAppointment(@RequestBody CancelAppointmentDto cancelDto) {
    try {
      if (cancelDto == null || cancelDto.getPatientCpr() == null || cancelDto.getPatientCpr().trim().isEmpty()) {
      ...
      CancelAppointmentRequest request = CancelAppointmentRequest.newBuilder()
          ...
          .setPatientCpr(cancelDto.getPatientCpr())
          .build();
      ...
      // Check response
      if (response.getConfirmation() == null || response.getConfirmation().isEmpty()) {
        return ResponseDto.createCancellationError("No confirmation received from server");
      }
      ...
      if (e.getStatus().getCode() == Status.Code.FAILED_PRECONDITION) {
        ...
        return ResponseDto.createCancellationError(e.getStatus().getDescription());
      }
      return ResponseDto.createCancellationError("Server error: " + e.getStatus().getDescription());
    } catch (Exception e) {
  ...
  public ResponseDto getAppointments(@RequestParam String cpr) {
    try {
      ...
      errorResponse.setResponse("Error fetching appointments: " + e.getStatus().getDescription());
      return errorResponse;
    } catch (Exception e) {

      ResponseDto errorResponse = new ResponseDto();
      errorResponse.setSuccess(false);
      errorResponse.setResponse("Error fetching appointments: " + e.getMessage());
      return errorResponse;
    }
  }
}
